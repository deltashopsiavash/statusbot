...
function panelDetailsText(panel) {
  ...
  return [
    ...
    `📉 حجم فروخته‌شده/ساخته‌شده دائمی: ${bytesToHuman(usage.usedBytes)}`,
...
function buildQuotaReportText(panel) {
  ...
  return [
    ...
    `📉 حجم فروخته‌شده/ساخته‌شده: ${bytesToHuman(usage.usedBytes)} (${bytesToGBNumber(usage.usedBytes)} GB)`,
...
function buildInboundSummaryText(panel, summary) {
  ...
  if (summary.quota) {
    ...
    lines.push(` حجم فروخته‌شده حتی حذف‌شده‌ها: ${bytesToHuman(summary.quota.usedBytes)} (${bytesToGBNumber(summary.quota.usedBytes)} GB)`);
