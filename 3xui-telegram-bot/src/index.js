...
function panelDetailsText(panel) {
  ...
  return [
    ...
    `📉 مصرف سهمیه از زمان ثبت مبنا: ${bytesToHuman(usage.usedBytes)}`,
    `🧾 فروش قدیمی قبل از مبنا: ${bytesToHuman(usage.baselineSoldBytes)}`,
...
function getPanelQuotaStats(panelId) {
  ...
  const row = db.prepare('SELECT COALESCE(SUM(max_used_bytes), 0) AS lifetimeSoldBytes, COUNT(1) AS totalClients, COALESCE(SUM(CASE WHEN active = 1 THEN 1 ELSE 0 END), 0) AS activeClients FROM panel_client_usage WHERE panel_id = ?').get(panelId);
  const limitBytes = Number(panel.traffic_limit_bytes || 0);
  const lifetimeSoldBytes = Number(row?.lifetimeSoldBytes || 0);
  const baselineSoldBytes = Number(panel.quota_baseline_sold_bytes || 0);
  const usedBytes = Math.max(lifetimeSoldBytes - baselineSoldBytes, 0);
  return {
    limitBytes,
    usedBytes,
    lifetimeSoldBytes,
...
function getPanelLifetimeSoldBytes(panelId) {
  const row = db.prepare('SELECT COALESCE(SUM(max_used_bytes), 0) AS lifetimeSoldBytes FROM panel_client_usage WHERE panel_id = ?').get(panelId);
  return Number(row?.lifetimeSoldBytes || 0);
}

function ensureQuotaBaseline(panelId) {
  const panel = getPanelById(panelId);
  if (!panel) return;
  if (Number(panel.traffic_limit_bytes || 0) <= 0) return;
  if (Number(panel.quota_baseline_set_at || 0) > 0) return;
  const lifetimeSoldBytes = getPanelLifetimeSoldBytes(panelId);
  db.prepare('UPDATE panels SET quota_baseline_sold_bytes = ?, quota_baseline_set_at = ? WHERE id = ?').run(lifetimeSoldBytes, Date.now(), panelId);
}
...
function syncPanelUsage(panelId, items) {
  ...
  const tx = db.transaction(() => {
    ...
    for (const item of items) {
      ...
      seen.add(clientKey);
      const soldBytes = calcSoldBytes(item.client, item.stat);
      ...
      if (old) {
        updateStmt.run(email, inboundId, inboundName, soldBytes, now, panelId, clientKey);
      } else {
        insertStmt.run(panelId, clientKey, email, inboundId, inboundName, soldBytes, now, now);
      }
    }
  });
  tx();
  ensureQuotaBaseline(panelId);
...
function buildQuotaReportText(panel) {
  ...
  return [
    ...
    `📉 مصرف سهمیه از زمان ثبت مبنا: ${bytesToHuman(usage.usedBytes)} (${bytesToGBNumber(usage.usedBytes)} GB)`,
    `🧾 فروش قدیمی قبل از مبنا: ${bytesToHuman(usage.baselineSoldBytes)} (${bytesToGBNumber(usage.baselineSoldBytes)} GB)`,
    `✅ باقی‌مانده: ${bytesToHuman(usage.remainingBytes)} (${bytesToGBNumber(usage.remainingBytes)} GB)`,
    ...(usage.usedBytes > usage.limitBytes && usage.limitBytes > 0 ? [`⚠️ اضافه‌فروشی نسبت به سهمیه: ${bytesToHuman(usage.usedBytes - usage.limitBytes)} (${bytesToGBNumber(usage.usedBytes - usage.limitBytes)} GB)`] : []),
...
function calcSoldBytes(client, stat) {
  // سهمیه پنل باید بر اساس حجمی که کاربر فروخته/ساخته کم شود،
  // نه مصرف واقعی up/down. پس total هر کلاینت ملاک است.
  return getClientTotalBytes(client, stat);
}
...
function buildInboundSummaryText(panel, summary) {
  ...
  if (summary.quota) {
    ...
    lines.push(` مصرف سهمیه از زمان ثبت مبنا: ${bytesToHuman(summary.quota.usedBytes)} (${bytesToGBNumber(summary.quota.usedBytes)} GB)`);
    lines.push(` فروش قدیمی قبل از مبنا: ${bytesToHuman(summary.quota.baselineSoldBytes)} (${bytesToGBNumber(summary.quota.baselineSoldBytes)} GB)`);
    lines.push(` باقی‌مانده از سهمیه: ${bytesToHuman(summary.quota.remainingBytes)} (${bytesToGBNumber(summary.quota.remainingBytes)} GB)`);
    if (summary.quota.usedBytes > summary.quota.limitBytes && summary.quota.limitBytes > 0) {
      const overBytes = summary.quota.usedBytes - summary.quota.limitBytes;
      lines.push(` ⚠️ اضافه‌فروشی نسبت به سهمیه: ${bytesToHuman(overBytes)} (${bytesToGBNumber(overBytes)} GB)`);
    }
...
bot.on('text', async (ctx) => {
  ...
  if (st.step === 'ASK_PANEL_PASS') {
    ...
    db.prepare('INSERT INTO panels (name, base_url, web_base_path, username, password, created_at, owner_tg_id, traffic_limit_bytes, quota_baseline_sold_bytes, quota_baseline_set_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
      st.data.name || '',
      st.data.base_url,
  ...
  if (st.step && st.step.startsWith('EDIT_')) {
    ...
    if (st.step === 'EDIT_QUOTA') {
      ...
      if (Number(panel.quota_baseline_set_at || 0) === 0 && bytes > 0) {
        patch.quota_baseline_sold_bytes = getPanelLifetimeSoldBytes(panel.id);
        patch.quota_baseline_set_at = Date.now();
      }
      if (bytes === 0) {
        patch.quota_baseline_sold_bytes = 0;
        patch.quota_baseline_set_at = 0;
      }
    }
    ...
    db.prepare(`
      UPDATE panels
      SET name = ?, base_url = ?, web_base_path = ?, username = ?, password = ?, owner_tg_id = ?, traffic_limit_bytes = ?, quota_baseline_sold_bytes = ?, quota_baseline_set_at = ?
      WHERE id = ?
    `).run(
      updatedPanel.name || '',
      ...
      Number(updatedPanel.quota_baseline_sold_bytes || 0),
      Number(updatedPanel.quota_baseline_set_at || 0),
      panel.id
    );
